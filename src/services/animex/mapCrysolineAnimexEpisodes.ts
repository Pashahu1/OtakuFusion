import type { CrysolineAnimexEpisodeRow } from '@/server/crysoline/animexClient';
...
function episodeHasSubProviders(row: CrysolineAnimexEpisodeRow): boolean {
  const list = row.metadata?.subProviders;
  return Array.isArray(list) && list.length > 0;
}

function episodeHasDubProviders(row: CrysolineAnimexEpisodeRow): boolean {
  const list = row.metadata?.dubProviders;
  return Array.isArray(list) && list.length > 0;
}

export function mapCrysolineAnimexEpisodes(
  rows: CrysolineAnimexEpisodeRow[]
): GetEpisodesResult {
...
export function seriesHasDubFromAnimexEpisodes(rows: CrysolineAnimexEpisodeRow[]): boolean {
