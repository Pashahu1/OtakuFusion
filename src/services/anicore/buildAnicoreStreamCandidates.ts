import type { CrysolineAnicoreSourcesPayload } from '@/server/crysoline/anicoreClient';
import {
  isDeadAnicoreCdnUrl,
...
function mapSubtitlesToTracks(
  subtitles: CrysolineAnicoreSourcesPayload['subtitles'],
  lang: WatchLang
): VideoTrack[] {
...
export function sortAnicoreCandidatesForProbe(candidates: StreamingType[]): StreamingType[] {
...
export function buildAnicoreStreamCandidates(
  payload: CrysolineAnicoreSourcesPayload,
  lang: WatchLang,
  serverLabel: string
): StreamingType[] {
  ...
  for (const row of sources) {
    ...
    out.push({
      ...
      tracks,
      server: qualityLabel !== 'Auto' ? qualityLabel : serverLabel || 'Anicore',
      request_headers: Object.keys(headers).length ? headers : undefined,
    });
  }

  return sortAnicoreCandidatesForProbe(out);
}
